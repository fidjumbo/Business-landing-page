...
const Navbar = () => {
  ...
  return (
    ...
    whileInView="show"
    viewport={{once: true}}
    className='fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-sm-50 z-50 border-gray-100 shadow-sm'>
        <div className='w-full container mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 md:h-20 h-16'>
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className='md:hidden p-2'>
            {
                isMenuOpen ? <HiX className='size-6 h-6'/> : <HiMenu className='size-6 h-6'/>
            }
          </button>
          ...
          <div className='hidden md:flex items-center gap-10'>
            {
                navLinks.map((link, index) => (
                    ...
                    after:w-0 hover:after:w-full after:bg-blue-600 after:transition-all ${activeLink === 
                        link.href ? "text-blue-600 after:w-full" : "text-gray-600 hover:text-gray-900"}`}>
                        {link.label}
                    </a>
                ))
            }
                    ...
                    <div className='container mx-auto px-4 space-y-3'>
                        {navLinks.map((link, index) => (
                            ...
                            key={index}
                             ...
                             className={`block text-sm font-medium py-2 ${activeLink === 
